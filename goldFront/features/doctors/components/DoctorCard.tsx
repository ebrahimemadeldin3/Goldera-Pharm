...
export default function DoctorCard({
  ...
  return (
    <>
      <Card
        className="group/doctor border-gp-border-default bg-gp-surface-card shadow-gp-card relative flex min-h-[286px] flex-col justify-between gap-4 overflow-hidden rounded-[16px] border p-4 opacity-0 transition-[border-color,box-shadow,transform] duration-[200ms] ease-out hover:-translate-y-0.5 hover:border-gp-gold-300 hover:shadow-[0_10px_26px_rgba(16,29,54,0.09)] [animation:plans-card-in_350ms_ease-out_forwards] motion-reduce:transform-none motion-reduce:opacity-100 motion-reduce:transition-none motion-reduce:[animation:none]"
        style={{ animationDelay: `${Math.min(index, 9) * 45}ms` }}
      >
        <span
          className="bg-gp-gold-500 absolute top-4 bottom-4 left-0 w-[3px] rounded-r-full"
          aria-hidden="true"
        />

        <div className="flex flex-col gap-4">
          <div className="flex items-start gap-3.5">
            <span className="border-gp-gold-300 bg-gp-navy-900 text-gp-gold-500 flex size-12 shrink-0 items-center justify-center rounded-[12px] border text-sm font-semibold shadow-[0_6px_14px_rgba(16,29,54,0.15)] transition-[border-color,transform] duration-[200ms] group-hover/doctor:border-gp-gold-500 group-hover/doctor:-translate-y-0.5 motion-reduce:transform-none">
              {initials}
            </span>

            <div className="min-w-0 flex-1">
              <h3
                className="text-gp-navy-900 truncate text-base leading-6 font-semibold transition-colors duration-[180ms] group-hover/doctor:text-gp-navy-900"
                dir="auto"
              >
                {primaryName}
              </h3>
        ...
        <div className="border-gp-border-subtle flex flex-col gap-2 border-t pt-3 sm:flex-row sm:justify-end">
          {features.visits.canScheduleVisit && (
            <div className="flex items-center">
              <Button
                ...
                className="group/schedule border-gp-gold-300 bg-gp-gold-50 text-gp-gold-700 hover:border-gp-gold-500 hover:bg-gp-surface-hover h-10 w-full cursor-pointer rounded-[10px] border px-3.5 text-xs font-semibold shadow-none transition-[background-color,border-color,color,box-shadow] duration-[180ms] focus-visible:ring-3 focus-visible:ring-gp-gold-500/20 focus-visible:outline-none sm:w-auto"
              >
                <CalendarDays
          ...
          {features.doctors.canView && (
            <Link
              href={profilePath}
              className="group/profile bg-gp-navy-900 hover:bg-gp-navy-900/95 inline-flex h-10 w-full items-center justify-center gap-1.5 rounded-[10px] px-3.5 text-xs font-semibold text-white shadow-[0_6px_16px_rgba(16,29,54,0.16)] transition-[background-color,box-shadow,transform] duration-[180ms] hover:-translate-y-0.5 hover:shadow-[0_10px_22px_rgba(16,29,54,0.2)] focus-visible:ring-3 focus-visible:ring-gp-gold-500/25 focus-visible:outline-none motion-reduce:transform-none sm:w-auto"
            >
              View Profile
              <ArrowRight
                className="size-3.5 text-gp-gold-500 transition-transform duration-[180ms] group-hover/profile:translate-x-0.5 motion-reduce:transition-none"
                aria-hidden="true"
              />
            </Link>
          )}
